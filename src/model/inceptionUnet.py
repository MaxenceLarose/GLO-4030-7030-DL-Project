...
class InceptionBlock(nn.Module):
	def __init__(self, in_channels, out_channels, middle_channels=None, reduce_channels=True, kernel_size=3, batch_norm_momentum=0.1):
		super().__init__()
		self.relu = nn.LeakyReLU(inplace=True)
		...
		# 1x1 convolutions
		# self.conv1x1_1 = nn.Conv2d(in_channels, middle_channels, kernel_size=1)
		# self.conv1x1_2 = nn.Conv2d(middle_channels, out_channels, kernel_size=1)
		# 3x3 convolutions
	...
	def forward(self, x):
		# out1x1 = self.conv1x1_1(x)
		# out1x1 = self.bn_1(out1x1)
		# out1x1 = self.relu(out1x1)
		# out1x1 = self.conv1x1_2(out1x1)
		# out1x1 = self.bn_2(out1x1)
		# out1x1 = self.relu(out1x1)
		...
		# out7x7 = self.conv7x7_1(tmp)
		# out7x7 = self.bn_7x7_1(out7x7)
		# out7x7 = self.relu(out7x7)
		# out7x7 = self.conv7x7_2(out7x7)
		# out7x7 = self.bn_7x7_2(out7x7)
		# out7x7 = self.relu(out7x7)
...
class InceptionUNet(nn.Module):
    def __init__(self, num_classes, input_channels=3, nb_filter=[32, 64, 128, 256, 512], batch_norm_momentum=0.1):
        ...
        self.conv0_0 = InceptionBlock(input_channels, nb_filter[0], nb_filter[0], batch_norm_momentum=0.1)
        self.conv1_0 = InceptionBlock(nb_filter[0], nb_filter[1], nb_filter[1], batch_norm_momentum=0.1)
        self.conv2_0 = InceptionBlock(nb_filter[1], nb_filter[2], nb_filter[2], batch_norm_momentum=0.1)
        self.conv3_0 = InceptionBlock(nb_filter[2], nb_filter[3], nb_filter[3], batch_norm_momentum=0.1)
        self.conv4_0 = InceptionBlock(nb_filter[3], nb_filter[4], nb_filter[4], batch_norm_momentum=0.1)

        self.conv3_1 = InceptionBlock(nb_filter[3]+nb_filter[4], nb_filter[3], nb_filter[3], batch_norm_momentum=0.1)
        self.conv2_2 = InceptionBlock(nb_filter[2]+nb_filter[3], nb_filter[2], nb_filter[2], batch_norm_momentum=0.1)
        self.conv1_3 = InceptionBlock(nb_filter[1]+nb_filter[2], nb_filter[1], nb_filter[1], batch_norm_momentum=0.1)
        self.conv0_4 = InceptionBlock(nb_filter[0]+nb_filter[1], nb_filter[0], nb_filter[0], batch_norm_momentum=0.1)
