...
from model.metrics import validate_2
		...
		for i_epoch in range(n_epoch):
			...
			with torch.enable_grad():
				for j, (inputs, targets) in enumerate(train_loader):
					...
					batch_loss.backward()
					opt.step()
					sys.stdout.write("%s[%s%s] %i/%i\r" % ("mini batch :", "#"*x, "."*(toolbar_width-x), j, n_batch))
					sys.stdout.flush()
					j += 1
			train_loss, train_RMSE = validate_2(network, train_loader, loss, use_gpu=use_gpu)
			valid_loss, valid_RMSE = validate_2(network, valid_loader, loss, use_gpu=use_gpu)
			t1 = time.time()
			...
			if valid_RMSE < best_RMSE:
				torch.save(network, "{}_best.pt".format(save_path))
...
if __name__ == '__main__':
	...
	if network_to_use not in available_networks:
		...
		preprocessing = None
	elif network_to_use is "InceptionUNet":
	...
	if load_data_for_challenge:
		train_images, test_images = load_all_images(image_types=["FBP", "Phantom", "Sinogram"], n_batch=1,
		...
		aapm_dataset = BreastCTDataset(train_images["FBP"][:1000], train_images["PHANTOM"][:1000], preprocessing=preprocessing)
